...
pub enum ParseError<'a> {
    ...
    MissingSymbol,
    #[snafu(display("'{}' is not a valid symbol", v))]
    InvalidSymbol { v: &'a str },
    #[snafu(display("general error"))]
    GeneralError,
}
...
// A production takes the form of
// a weight preceding the symbol name,
// followed by one or more sequences of symbols or string literals,
// with the sequences separated by `|`.
//
// For example:
//
//     3 job => location worker_type | "head" worker_type | "accountant"
...
// A directive takes the form `@name(arg1, arg2)`
...
// A language line is either a directive or a production or a comment
...
mod tests {
    ...
    fn test_parse_directive() {
        ...
        let (directive, _rest) = parse::parse_directive(directive_raw).unwrap();
        assert_eq!(
            parse::Directive {
